...
def FIDM(
    ...
    with torch.no_grad():
        ...
        # --- [Innovation]: SLIC-inspired Spatial-Semantic Joint Score ---
        metric = metric / metric.norm(dim=-1, keepdim=True)
        a, b = split(metric)
        feature_sim = a @ b.transpose(-1, -2) # [B, N-num_dst, num_dst]

        # 1. Generate normalized 2D spatial coordinates for all tokens [N, 2]
        device = metric.device
        grid_y, grid_x = torch.meshgrid(torch.arange(h, device=device), torch.arange(w, device=device), indexing='ij')
        coords = torch.stack([grid_x, grid_y], dim=-1).reshape(N, 2).float()
        coords[:, 0] = coords[:, 0] / max(w - 1, 1) # Normalize X to [0, 1]
        coords[:, 1] = coords[:, 1] / max(h - 1, 1) # Normalize Y to [0, 1]
        coords = coords.unsqueeze(0).expand(B, N, 2)
        
        # 2. Split coordinates using the existing src (a) and dst (b) indices
        coord_a = gather(coords, dim=1, index=a_idx.expand(B, N - num_dst, 2))
        coord_b = gather(coords, dim=1, index=b_idx.expand(B, num_dst, 2))
        
        # 3. Calculate L2 spatial distance: [B, N-num_dst, num_dst]
        spatial_dist = torch.cdist(coord_a, coord_b, p=2) 
        
        # 4. Combine spatial penalty with semantic similarity
        #    alpha is similar to the 'compactness' parameter (m) in SLIC
        alpha = tore_info.get("args", {}).get("slic_alpha", 0.5) if tore_info else 0.5
