...
from taylor_cache import (
    taylor_cal_type,
    taylor_cache_init,
    taylor_derivative_approximation,
    taylor_formula,
)
...
'''
每次生成前，重置并准备 SDTM + TaylorSeer 的全局状态。
'''
def make_SDTM_TaylorSeer_pipe(pipe_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:

    class StableDiffusion3Pipeline_SDTM_TaylorSeer(pipe_class):
        ...
        def __call__(self, *args, **kwargs):
            ...
            # Reset Taylor cache state for each new generation
            taylor = self._tore_info.setdefault("taylor", {})
            taylor["cache_counter"] = 0
            taylor["activated_steps"] = []
            taylor["cache"] = {}
            output = super().__call__(*args, **kwargs)
            return output

    return StableDiffusion3Pipeline_SDTM_TaylorSeer
'''
每个 step 决定策略开关 + Taylor 类型判断
'''
def make_SDTM_TaylorSeer_model(model_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    
    class SD3Transformer2DModel_SDTM_TaylorSeer(model_class):
        _parent = model_class
        ...
        def forward(self, *args, **kwargs):
            ...
            # ---- TaylorSeer: determine if this step is "full" or "Taylor" ----
            taylor_type = taylor_cal_type(self._tore_info)
            self._tore_info["states"]["taylor_type"] = taylor_type

            if taylor_type == "full":
                # Full computation step: apply SDTM merge strategy as usual
                if self._tore_info["states"]["step_current"] <= self._tore_info["args"]["switch_step"]:
                    ...
                    self._tore_info["states"]["merge_mlp"] = True
            # For "Taylor" type, merge settings don't matter (computation is skipped)

            output = super().forward(*args, **kwargs)
            return output

    return SD3Transformer2DModel_SDTM_TaylorSeer
'''
每层真正执行：Taylor近似跳过 / full+protected / full+SDTM merge/unmerge
'''
def make_SDTM_TaylorSeer_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:

    class JointTransformerBlock_SDTM_TaylorSeer(block_class):
        ...
        def forward(
            ...
            taylor_type = self._tore_info["states"].get("taylor_type", "full")

            # ================================================================
            # Taylor step: skip all heavy computation, use Taylor expansion
            # to approximate the residuals that would have been added.
            # ================================================================
            if taylor_type == "Taylor":
                # Approximate hidden_states residuals from Taylor cache
                attn_approx = taylor_formula(self._tore_info, layer_current, "attn")
                if attn_approx is not None:
                    hidden_states = hidden_states + attn_approx

                mlp_approx = taylor_formula(self._tore_info, layer_current, "mlp")
                if mlp_approx is not None:
                    hidden_states = hidden_states + mlp_approx

                # Approximate encoder_hidden_states residuals
                if self.context_pre_only:
                    encoder_hidden_states = None
                else:
                    ctx_attn_approx = taylor_formula(self._tore_info, layer_current, "context_attn")
                    if ctx_attn_approx is not None:
                        encoder_hidden_states = encoder_hidden_states + ctx_attn_approx

                    ctx_mlp_approx = taylor_formula(self._tore_info, layer_current, "context_mlp")
                    if ctx_mlp_approx is not None:
                        encoder_hidden_states = encoder_hidden_states + ctx_mlp_approx

                return encoder_hidden_states, hidden_states

            # ================================================================
            # Full step: actual computation with SDTM merge + Taylor cache update
            # ================================================================
            ...
            if protected_step:
                ...
                #! TaylorSeer: cache attn residual for Taylor expansion
                taylor_cache_init(self._tore_info, layer_current, "attn")
                taylor_derivative_approximation(self._tore_info, layer_current, "attn", attn_output)
                ...
                #! TaylorSeer: cache mlp residual for Taylor expansion
                taylor_cache_init(self._tore_info, layer_current, "mlp")
                taylor_derivative_approximation(self._tore_info, layer_current, "mlp", ff_output)
                ...
                #! TaylorSeer: cache attn residual (after unmerge) for Taylor expansion
                taylor_cache_init(self._tore_info, layer_current, "attn")
                taylor_derivative_approximation(self._tore_info, layer_current, "attn", attn_output)
                hidden_states = hidden_states + attn_output
                ...
                #! TaylorSeer: cache mlp residual (after unmerge) for Taylor expansion
                taylor_cache_init(self._tore_info, layer_current, "mlp")
                taylor_derivative_approximation(self._tore_info, layer_current, "mlp", ff_output)
                hidden_states = hidden_states + ff_output
            ...
            if self.context_pre_only:
                ...
                # Taylor cache: context attention residual
                taylor_cache_init(self._tore_info, layer_current, "context_attn")
                taylor_derivative_approximation(self._tore_info, layer_current, "context_attn", context_attn_output)
                ...
                context_mlp_residual = c_gate_mlp.unsqueeze(1) * context_ff_output
                # Taylor cache: context MLP residual
                taylor_cache_init(self._tore_info, layer_current, "context_mlp")
                taylor_derivative_approximation(self._tore_info, layer_current, "context_mlp", context_mlp_residual)
                encoder_hidden_states = encoder_hidden_states + context_mlp_residual
                
            return encoder_hidden_states, hidden_states
    
    return JointTransformerBlock_SDTM_TaylorSeer

def apply_SDTM_TaylorSeer(
    ...
    taylor_first_enhance: int = 2,
):

    # Make sure the module is not currently patched
    remove_patch(pipe)
    make_pipe_fn = make_SDTM_TaylorSeer_pipe
    pipe.__class__ = make_pipe_fn(pipe.__class__)
    pipe._tore_info = {
        "type": "SDTM_TaylorSeer",
        "args": {
            "ratio": ratio,
            "deviation": deviation,
        ...
        "taylor": {
            "interval": taylor_interval,
            "max_order": taylor_max_order,
            "first_enhance": taylor_first_enhance,
            "cache_counter": 0,
            "activated_steps": [],
            "cache": {},
        },
    ...
    make_model_fn = make_SDTM_TaylorSeer_model
    ...
    for _, module in model.named_modules():
        if isinstance_str(module, "JointTransformerBlock"):
            make_block_fn = make_SDTM_TaylorSeer_block
