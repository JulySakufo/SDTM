...
def taylor_cal_type(tore_info: Dict) -> str:
    ...
    # Protect first and last `first_enhance` steps as full computation.
    # This aligns with TaylorSeer-Only scheduling and improves stability/FID.
    first_steps = step_current < first_enhance
    last_steps = step_current >= max(step_count - first_enhance, 0)
...
def taylor_cache_init(tore_info: Dict, layer: int, module_name: str):
    ...
    # On the very first activated step (step 0 in ascending order), create an empty dict
    if step_current == 0:
...
def taylor_derivative_approximation(
    ...
    for i in range(max_order):
        # Skip derivative computation on the very first activated step (not enough data)
        # In ascending order, step 0 is the first step; we need at least 2 activated steps
        if i in old and step_current >= first_enhance:
